...
class MainTest {
    ...
    public void RESP_1_test_1(){
        ...
        for (int i = 0; i < 50; i++){
            assertEquals("Foe", adventureDeck.cards[i].getType());
            if(i < 8){ //checking for 5(8)
                assertEquals(5, adventureDeck.cards[i].getValue());
            }
            else if(i < 15){ //checking for 10(7)
                assertEquals(10, adventureDeck.cards[i].getValue());
            }
            else if(i < 23){ //checking for 15(8)
                assertEquals(15, adventureDeck.cards[i].getValue());
            }
            else if(i < 30){ //checking for 20(7)
                assertEquals(20, adventureDeck.cards[i].getValue());
            }
            else if(i < 37){ //checking for 25(7)
                assertEquals(25, adventureDeck.cards[i].getValue());
            }
            else if(i < 41){ //checking for 30(4)
                assertEquals(30, adventureDeck.cards[i].getValue());
            }
            else if(i < 45){ //checking for 35(4)
                assertEquals(35, adventureDeck.cards[i].getValue());
            }
            else if(i < 47){ //checking for 40(2)
                assertEquals(40, adventureDeck.cards[i].getValue());
            }
            else if(i < 49){ //checking for 50(2)
                assertEquals(50, adventureDeck.cards[i].getValue());
            }
            else if(i < 50){ //checking for 70(1)
                assertEquals(70, adventureDeck.cards[i].getValue());
            }
        }
        for (int i = 50; i < 100; i++){
            assertEquals("Weapon", adventureDeck.cards[i].getType());
        ...
        for (int i = 0; i < 12; i++) {
            assertEquals("Quest", eventDeck.cards[i].getType());
            if(i < 3){ //checking for 3 Q2
                assertEquals(2, eventDeck.cards[i].getValue());
            }
            else if(i < 7){ //checking for 4 Q3
                assertEquals(3, eventDeck.cards[i].getValue());
            }
            else if(i < 10){ //checking for 3 Q4
                assertEquals(4, eventDeck.cards[i].getValue());
            }
            else if(i < 12){ //checking for 2 Q5
                assertEquals(5, eventDeck.cards[i].getValue());
            }
        }
        for (int i = 12; i < 17; i++) {
            assertEquals("Event", eventDeck.cards[i].getType());
            if(i < 13){ //checking for  1 “Plague”
                assertEquals("Plague", eventDeck.cards[i].getValue());
            }
            else if(i < 15){ //checking for  2 “Queen’s favor”
                assertEquals("Queen's favor", eventDeck.cards[i].getValue());
            }
            else if(i < 17){ //checking for 2 “Prosperity”
                assertEquals("Prosperity", eventDeck.cards[i].getValue());
            }
        }
    }
}